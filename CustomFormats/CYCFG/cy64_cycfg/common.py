...
class CYCFG__V1:
    ...
    def __init__(self, main, off, fd):
        ...
        self.name = self.name.decode("utf-16-le","ignore").strip('\0')
        keyCount = main.keys
        main.keys = {}
    ...
    def compile(self, fd, main):
        ...
        if dataOff&15: dataOff += 16-(dataOff&15)
    
        fd.write(pack(
            self.__STRUCT__, self.name.encode("utf-16-le","replace"),
            dataOff, *self.reserved
        ))
        
        tbOff = fd.tell()
        
        keys = list(main.keys.keys())
        keys.sort()
        
        for i in keys:
            fd.seek(tbOff)
            keyName = i.encode("utf-8","replace")+bytes((0,))
            if len(keyName)&3: keyName += b'\0'*(4-(len(keyName)&3))
            data = CYCFGTypeCnv.compileKey(*main.keys[i])
            if len(data)&3: data += b'\0'*(4-(len(data)&3))
            fd.write(pack(
...
class CYCFG:
    ...
    def __init__(self, data=None, failOnCRC=True):
        self.keys = 0
        self.cfg = None
        self.closed = True
        self.version = 0
        
        if data: self.decompile(data, failOnCRC)
    ...
    def compile(self, out=None):
        ...
        s = fd.tell()
        if s&15: fd.write(b'\0'*(16-(s&15)))
        fd.write(b'\0'*12)
        s = fd.tell()
        fd.seek(0)
        fd.write(pack(
            self.__HEADSTRUCT__, b'CY64%CFG', self.version, len(self.keys), s + 4
        ))
        fd.seek(s)
        fd.write(int.to_bytes(CRC32(fd.getvalue()).digest(),4,"little"))
    ...
    def decompile(self, data, failOnCRC=True):
        if type(data)==str:
            fd = open(data, "rb")
            self.closed = False
        ...
        try:
            ...
            if self.version == 1:
                self.cfg = CYCFG__V1(
                    self, fd.tell() - self.offset, io.BytesIO(fd.read(self.size - calcsize(self.__HEADSTRUCT__)))
                )
            else:
                raise Exception("Unknown CYCFG version: "+str(self.version))
            fd.seek(self.offset)
            buf = fd.read(self.size - 4)
            crcOK = CRC32(buf).digest() == int.from_bytes(fd.read(4),"little")
            if not crcOK:
                if failOnCRC: raise Exception("Bad CRC checksum")
    ...
    def print(self):
        ...
        for i in keys:
            print(f"     '{i.ljust(24)}': [{self.keys[i][0].name.ljust(6)}, {self.keys[i][1]}]")
        print(f"   ]")
        print("}")
